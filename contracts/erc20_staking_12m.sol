...
contract TokenStaking {
    ...
    function stake(uint256 _value) external payable ifNotPaused {
        ...
        ownerToReleaseTime[msg.sender] = block.timestamp + (12 * 2628000);  // one month equals 192 000 blocks
    ...
    function cancel() external payable ifNotPaused {
        ...
        if (block.timestamp >= ownerToReleaseTime[msg.sender]) {
            ...
            require(ownerToStakedValue[msg.sender] > 0, "Staked value is zero");

            uint256 weeks_left = (ownerToReleaseTime[msg.sender] - block.timestamp) / 604800;
            uint256 amountAfterFine = ownerToStakedValue[msg.sender] * 95 / 100;
            if (weeks_left >= 39) {
                amountAfterFine = ownerToStakedValue[msg.sender] * 75 / 100;
            } else if (weeks_left >= 26) {
                amountAfterFine = ownerToStakedValue[msg.sender] * 80 / 100;
            } else if (weeks_left >= 13) {
                amountAfterFine = ownerToStakedValue[msg.sender] * 85 / 100;
            } else if (weeks_left >= 4) {
                amountAfterFine = ownerToStakedValue[msg.sender] * 90 / 100;
            }

            require(amountAfterFine > 0, "Amount after fine is zero");
            token.transfer(msg.sender, amountAfterFine);
        }
    }
